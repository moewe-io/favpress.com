 ...
 * and compatible with all Fafpress Framework Option Controls.
 */
...
class FavPress_MetaBox_Alchemy extends WPAlchemy_MetaBox
{
	...
	function _setup()
	{
		...
		else
		{
			...
			echo '<div class="favpress-metabox">';
			$this->_enview($fields);
			echo '</div>';
		}
		...
		foreach ($fields as $name => $field)
		{
			...
			else
			{
				...
				if(!empty($bind) and is_null($val))
				{
					...
					if(FavPress_Util_Reflection::is_multiselectable($field))
					{
						$result = (array) $result;
					}
					else
				...
				if($field instanceof FavPress_Control_FieldMulti)
				{
		...
		if(!function_exists('loop_fields'))
		{
			function loop_fields(&$fields)
			{
				foreach ($fields as &$field)
				{
					...
					$dependency = '';
					if($field instanceof FavPress_Control_Field)
					{
					...
					if(!empty($dependency))
					{
						...
						if(!$result)
						{
							if($field instanceof FavPress_Control_Field)
							{
								...
								if($field->is_hidden())
									$field->add_container_extra_classes('favpress-hide');

								$field->add_container_extra_classes('favpress-dep-inactive');
							}
							else
							{
								$field['is_hidden'] = true;
	...
	function _enfactor_field($field, $mb, $in_group = false)
	{
		...
		// create the object
		$make     = FavPress_Util_Reflection::field_class_from_type($field['type']);
		$favpress_field = call_user_func("$make::withArray", $field);

		// get value from mb
		$value    = $mb->get_the_value();
		// get default from array
		$default  = $favpress_field->get_default();
		...
		else
		{
			...
			{
				if( FavPress_Util_Reflection::is_multiselectable($field['type']) )
				{
					if( !is_array($value) )
						$value = array( $value );
				}
		...
		$favpress_field->set_value($value);

		if (!$in_group)
		{
			$favpress_field->add_container_extra_classes(array('favpress-meta-single'));
		}
	...
	function _enfactor_group($field, $mb, $repeating)
	{
		...
		if(isset($group['sortable']) and $group['sortable'])
			$group['container_extra_classes'][] = 'favpress-sortable';
	...
	function _render_group($group)
	{
		...
		$html .= '<div id="wpa_loop-' . $uid
				. '" class="favpress-wpa-loop level-' . $oddity . ' wpa_loop wpa_loop-' . $name . ' favpress-fixed-loop favpress-meta-group'
				. (isset($group['container_extra_classes']) ? (' ' . implode(' ', $group['container_extra_classes'])) : '')
				. '"'
				. FavPress_Util_Text::return_if_exists(isset($dependency) ? $dependency : '', 'data-favpress-dependency="%s"')
				. '>';
	...
	function _render_repeating_group($group)
	{
		...
		$html  = '';
		$html .= '<div id="wpa_loop-' . $uid
				. '" class="favpress-wpa-loop level-' . $oddity . ' wpa_loop wpa_loop-' . $name . ' favpress-repeating-loop favpress-meta-group'
				. (isset($group['container_extra_classes']) ? (' ' . implode(' ', $group['container_extra_classes'])) : '')
				. '"'
				. FavPress_Util_Text::return_if_exists(isset($dependency) ? $dependency : '', 'data-favpress-dependency="%s"')
				. '>';
		...
		foreach ($group['groups'] as $g)
		{
			...
			$html .= '<div class="favpress-wpa-group-heading"><a href="#" class="favpress-wpa-group-title">' . $icon . $group['title'] . '</a><a href="#" class="dodelete favpress-wpa-group-remove" title="'. __('Remove', '__PLUGIN_SLUG__') .'"><i class="fa fa-times"></i> '. __('Remove', '__PLUGIN_SLUG__') .'</a></div>';
			$html .= '<div class="favpress-controls' . ((!$is_first) ? ' favpress-hide' : '') . '">';
		...
		$html .= '<div class="favpress-wpa-group-add">';
		$html .= '<a href="#" class="button button-large docopy-' . $name . '">'. __('Add More', '__PLUGIN_SLUG__') . '' . $group['title'] . '</a>';
		$html .= '</div>';
	...
	function _save($post_id) 
	{
		...
		$dev_mode = FavPress_Util_Config::instance()->load('metabox', 'dev_mode');
		if($dev_mode)
			return;
