...
function favpress_get_categories()
{
...
function favpress_get_users()
{
    $wp_users = FavPress_WP_User::get_users();
...
function favpress_get_roles()
{
    $result = array();
    $editable_roles = FavPress_WP_User::get_editable_roles();
...
function favpress_get_gwf_family()
{
...
FavPress_Security::instance()->whitelist_function('favpress_get_gwf_weight');

function favpress_get_gwf_weight($face)
{
    if (empty($face))
        return array();
...
FavPress_Security::instance()->whitelist_function('favpress_get_gwf_style');

function favpress_get_gwf_style($face)
{
    if (empty($face))
        return array();
...
FavPress_Security::instance()->whitelist_function('favpress_get_gwf_subset');

function favpress_get_gwf_subset($face)
{
    if (empty($face))
        return array();
...
function favpress_get_social_medias()
{
    $socmeds = array(
...
function favpress_get_fontawesome_icons()
{
    // scrape list of icons from fontawesome css
    if (false === ($icons = get_transient('favpress_fontawesome_icons'))) {
        $pattern = '/\.(fa-(?:\w+(?:-)?)+):before\s*{\s*content/';
        $subject = file_get_contents(FAVPRESS_DIR . '/public/css/vendor/font-awesome.min.css');
        ...
        set_transient('favpress_fontawesome_icons', $icons, 60 * 60 * 24);
    }

    return $icons;
}

FavPress_Security::instance()->whitelist_function('favpress_dep_boolean');

function favpress_dep_boolean($value)
{
    $args = func_get_args();
    $result = true;
