...
class FavPress_Option_Control_Set
{
	...
	public function render()
	{
		// Setup data
		$data = array('set' => $this);
		return FavPress_View::instance()->load('option/set', $data);
	}
	...
	public function get_fields($include_section = false)
	{
		if(!function_exists('loop_controls'))
		{
			function loop_controls($menu, $include_section)
			{
				...
				{
					if( get_class($control) === 'FavPress_Option_Control_Group_Section' )
					{
						if($include_section)
						{
						...
						{
							if( FavPress_Util_Reflection::field_type_from_class(get_class($field)) != 'impexp' )
							{
					...
					{
						if( FavPress_Util_Reflection::field_type_from_class(get_class($control)) != 'impexp' )
						{
							$fields[$control->get_name()] = $control;
						}
					}
	...
	public function get_field_types()
	{
		...
		{
			$type = FavPress_Util_Reflection::field_type_from_class(get_class($field));
	...
	public function normalize_values($opt_arr)
	{
		...
		foreach ($opt_arr as $key => $value)
		{
			...
			{
				$is_multi = FavPress_Util_Reflection::is_multiselectable($fields[$key]);
	...
	public function save($option_key)
	{
		...
		do_action('favpress_option_set_before_save', $opt);
		...
		do_action('favpress_option_set_after_save', $curr_opt, $result['status'], $option_key);
	...
	public function populate_values($opt, $force_update = false)
	{
		...
		foreach ( $fields as $field )
		{
			$is_multi = FavPress_Util_Reflection::is_multiselectable($field);
