...
class FavPress_Option
{
	...
	public function __construct(array $configs)
	{
		...
		$configs = apply_filters('favpress_option_configuration_array'  , $configs, $configs['option_key']);
		$configs = apply_filters('favpress_option_configuration_array-' . $configs['option_key'], $configs);
		...
		add_action('favpress_option_first_activation', array($this, 'initial_db_setup'));

		// check if option key not existed init data from default values
		$options = get_option( $this->get_option_key() );
		if( $options === FALSE )
		{
			do_action('favpress_option_first_activation');
		}
		...
		// setup ajax
		add_action('wp_ajax_favpress_ajax_' . $this->get_option_key() . '_export_option', array($this, 'ajax_export_option'));
		add_action('wp_ajax_favpress_ajax_' . $this->get_option_key() . '_import_option', array($this, 'ajax_import_option'));
		add_action('wp_ajax_favpress_ajax_' . $this->get_option_key() . '_save'         , array($this, 'ajax_save'));
		add_action('wp_ajax_favpress_ajax_' . $this->get_option_key() . '_restore'      , array($this, 'ajax_restore'));
	...
	{	
		$opt_loader = FavPress_WP_Loader::instance();
		$opt_loader->add_types( $this->get_field_types(), 'option' );
		$opt_loader->add_main_js( 'favpress-option' );
		$opt_loader->add_main_css( 'favpress-option' );
		$opt_loader->add_js_data( 'favpress-option', 'custom_local.name', $this->_option_key );
		$opt_loader->add_js_data( 'favpress-option', 'custom_local.SAVE_SUCCESS', FavPress_Option_Control_Set::SAVE_SUCCESS );
		$opt_loader->add_js_data( 'favpress-option', 'custom_local.SAVE_NOCHANGES', FavPress_Option_Control_Set::SAVE_NOCHANGES );
		$opt_loader->add_js_data( 'favpress-option', 'custom_local.SAVE_FAILED', FavPress_Option_Control_Set::SAVE_FAILED );
	}
	...
	function save_and_reinit()
	{
		...
		do_action('favpress_option_save_and_reinit', $opt, $result['status'], $this->get_option_key());

		// option key specific save and re-init action hook
		do_action('favpress_option_save_and_reinit-' . $this->get_option_key(), $opt, $result['status']);
	...
	function ajax_save()
	{
		$result = $this->favpress_verify_nonce();
		...
		if($result['status'])
		{
			...
			$option  = FavPress_Util_Array::unite( $option, 'name', 'value' );
			...
			do_action('favpress_option_before_ajax_save', $opt);

			// save and re-init options
			$result = $this->save_and_reinit();

			// after ajax save action hook
			do_action('favpress_option_after_ajax_save', $opt, $old_opt, $result['status'], $this->get_option_key());

			// option key specific after ajax save action hook
			do_action('favpress_option_after_ajax_save-' . $this->get_option_key(), $opt, $old_opt, $result['status']);
		}
	...
	function ajax_restore()
	{
		...
		if( $result['status'] )
		{
			...
			// before ajax save action hook
			do_action('favpress_option_before_ajax_restore', $options);
			...
			do_action('favpress_option_after_ajax_restore', $options, $old_opt, $result['status'], $this->get_option_key());

			// after ajax restore action hook
			do_action('favpress_option_after_ajax_restore-' . $this->get_option_key(), $options, $old_opt, $result['status']);
		}
	...
	{
		global $favpress_set, $favpress_config;
		...
		do_action('favpress_option_after_ajax_import', $options, $old_opt, $result['status'], $this->get_option_key());

		// after ajax import action hook
		do_action('favpress_option_after_ajax_import-' . $this->get_option_key(), $options, $old_opt, $result['status']);
	...
	function ajax_export_option()
	{
		...
		do_action('favpress_option_after_ajax_export', $db_options, $sr_options, $result['status'], $this->get_option_key());

		// after ajax export action hook
		do_action('favpress_option_after_ajax_export-' . $this->get_option_key(), $db_options, $sr_options, $result['status']);
	...
	function favpress_verify_nonce()
	{
		$nonce  = $_POST['nonce'];
		$verify = check_ajax_referer('favpress', 'nonce', false);
		if($verify)
		{
			$result['status']  = true;
	...
	function initial_db_setup()
	{
		...
		do_action('favpress_option_before_db_init', $opt);

		// save to db
		$result = $this->save_and_reinit();

		// after db options db action hook
		do_action('favpress_option_after_db_init', $opt, $result['status'], $this->get_option_key());
		do_action('favpress_option_after_db_init-' . $this->get_option_key(), $opt, $result['status']);
	}
	...
	public function init_options_set()
	{
		...
		if( $this->use_util_menu() )
		{
			...
			$restore_button = new FavPress_Option_Control_Field_Restore();
			...
			// setup exim field
			$exim_field = new FavPress_Option_Control_Field_ImpExp();
	...
	// @todo return `favpress_option` like function
